...
def month_keyboard():
    ...
    for i in range(MONTHS_LOOKAHEAD):
        ...
        row.append(
            InlineKeyboardButton(
                month_name.capitalize(), callback_data=f"month_{month}_{current_year}"
            )
        )
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                INLINE_BUTTONS["BACK"], callback_data="back_to_procedures"
            )
        ]
    )
...
def date_keyboard(year: int, month: int, available_dates: List[date]):
    """Клавиатура для выбора даты в указанном месяце."""
    ...
    if not (month == end_month and year == end_year):
        ...
        header.append(
            InlineKeyboardButton(
                EMOJI["FORWARD"], callback_data=f"next_month_{next_year}_{next_month}"
            )
        )
...
def time_keyboard(available_slots: List[Tuple[time, time]]):
...
def procedures_keyboard():
    """Клавиатура для выбора процедуры."""
